...
class CriticSpider(CrawlSpider):
    ...
    def parse_critic_list_page(self, response):
        ...
        for row_sel in sel.xpath('.//tr'):
            critic_url = row_sel.xpath('.//td/p/a/@href').get().strip()
    ...
    def parse_critic_page(self, response):
        ...
        page_num = re.findall(self.page_num_regexp, response.url)
        sel = response.selector.xpath('//tbody[@id="review-table-body"]')
        if len(sel.xpath('.//tr')) == 0:
            return None
        for row_sel in sel.xpath('.//tr'):
            movie_url = row_sel.xpath('.//td[contains(@class, "critic-review-table__title-column")]/a/@href').get().strip()
